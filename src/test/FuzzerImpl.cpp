...
struct xdr_fuzzer_compactor
{
    ...
    typename std::enable_if<
        (!std::is_same<diamnet::AccountID, T>::value &&
         !std::is_same<diamnet::MuxedAccount, T>::value &&
         !std::is_same<diamnet::Asset, T>::value &&
         !std::is_same<diamnet::AssetCode, T>::value &&
         !std::is_same<diamnet::ClaimableBalanceID, T>::value &&
         !std::is_same<diamnet::LedgerKey, T>::value) &&
    ...
    template <typename T>
    typename std::enable_if<std::is_same<diamnet::AccountID, T>::value>::type
    operator()(T const& pk)
    {
        // convert public key 1 byte
        check(1);
        auto b = diamnet::FuzzUtils::getShortKey(pk.ed25519());
        put_bytes(&b, 1);
    }
    template <typename T>
    typename std::enable_if<std::is_same<diamnet::MuxedAccount, T>::value>::type
    operator()(T const& m)
    {
        // convert MuxedAccount -> 1 byte (same than an AccountID)
        auto const& ed25519 = (m.type() == diamnet::KEY_TYPE_ED25519)
                                  ? m.ed25519()
                                  : m.med25519().ed25519;
        check(1);
        auto b = diamnet::FuzzUtils::getShortKey(ed25519);
        put_bytes(&b, 1);
    }

    template <typename T>
    typename std::enable_if<std::is_same<diamnet::Asset, T>::value>::type
    operator()(T const& asset)
    {
        // Convert Asset to 1 byte.
        check(1);
        auto b = diamnet::FuzzUtils::getShortKey(asset);
        put_bytes(&b, 1);
    }

    template <typename T>
    typename std::enable_if<std::is_same<diamnet::AssetCode, T>::value>::type
    operator()(T const& code)
    {
        // Convert AssetCode to 1 byte.
        check(1);
        auto b = diamnet::FuzzUtils::getShortKey(code);
        put_bytes(&b, 1);
    }

    template <typename T>
    typename std::enable_if<
        std::is_same<diamnet::ClaimableBalanceID, T>::value>::type
    operator()(T const& balanceID)
    {
        ...
        check(1);
        auto b = diamnet::FuzzUtils::getShortKey(balanceID);
        put_bytes(&b, 1);
    }

    template <typename T>
    typename std::enable_if<std::is_same<diamnet::LedgerKey, T>::value>::type
    operator()(T const& key)
    {
        ...
        check(1);
        auto b = diamnet::FuzzUtils::getShortKey(key);
        put_bytes(&b, 1);
    }
};
...
{
    diamnet::FuzzUtils::StoredLedgerKeys mStoredLedgerKeys;
    diamnet::FuzzUtils::StoredPoolIDs mStoredPoolIDs;
    std::uint8_t const* mCur;
    std::uint8_t const* const mEnd;

    xdr_fuzzer_unpacker(
        diamnet::FuzzUtils::StoredLedgerKeys const& storedLedgerKeys,
        diamnet::FuzzUtils::StoredPoolIDs const& storedPoolIDs,
    ...
    typename std::enable_if<
        (!std::is_same<diamnet::AccountID, T>::value &&
         ...
         !std::is_same<diamnet::AssetCode, T>::value &&
         !std::is_same<diamnet::ClaimableBalanceID, T>::value &&
         !std::is_same<diamnet::LiquidityPoolDepositOp, T>::value &&
         !std::is_same<diamnet::LiquidityPoolWithdrawOp, T>::value &&
         !std::is_same<diamnet::LedgerKey, T>::value) &&
        (xdr_traits<T>::is_class || xdr_traits<T>::is_container)>::type
    operator()(T& t)
    {
    ...
    operator()(T& pk)
    {
        ...
        diamnet::FuzzUtils::setShortKey(pk, v);
    }
    template <typename T>
    typename std::enable_if<std::is_same<diamnet::MuxedAccount, T>::value>::type
    operator()(T& m)
    {
        ...
        check(1);
        std::uint8_t v = get_byte();
        diamnet::FuzzUtils::setShortKey(m.ed25519(), v);
    }
    ...
    operator()(T& asset)
    {
        ...
        check(1);
        std::uint8_t v = get_byte();
        asset = diamnet::FuzzUtils::makeAsset(v);
    }
    ...
    operator()(T& code)
    {
        ...
        check(1);
        std::uint8_t v = get_byte();
        code = diamnet::FuzzUtils::makeAssetCode(v);
    }
    ...
    operator()(T& balanceID)
    {
        check(1);
        std::uint8_t v = get_byte();
        diamnet::LedgerKey key;
        diamnet::FuzzUtils::setShortKey(mStoredLedgerKeys, key, v);
        ...
        if (key.type() == diamnet::CLAIMABLE_BALANCE)
        {
            balanceID = key.claimableBalance().balanceID;
        }
        else
        {
            balanceID.type(diamnet::CLAIMABLE_BALANCE_ID_TYPE_V0);
            balanceID.v0()[0] = v;
        }
    }
    ...
    template <typename T>
    typename std::enable_if<
        std::is_same<diamnet::LiquidityPoolDepositOp, T>::value>::type
    operator()(T& depositOp)
    {
        check(1);
        auto v = get_byte();
        diamnet::FuzzUtils::setShortKey(mStoredPoolIDs,
        ...
        depositOp.minPrice = diamnet::Price{minN, minD};
        depositOp.maxPrice = diamnet::Price{maxN, maxD};
    }

    template <typename T>
    typename std::enable_if<
        std::is_same<diamnet::LiquidityPoolWithdrawOp, T>::value>::type
...
xdr_from_fuzzer_opaque(
    diamnet::FuzzUtils::StoredLedgerKeys const& storedLedgerKeys,
    diamnet::FuzzUtils::StoredPoolIDs const& storedPoolIDs, Bytes const& m,
...
generator_t::operator()(diamnet::PublicKey& t) const
{
    ...
    diamnet::FuzzUtils::setShortKey(
        t.ed25519(), static_cast<uint8_t>(diamnet::rand_uniform<int>(
...
TransactionFuzzer::storeSetupLedgerKeysAndPoolIDs(AbstractLedgerTxn& ltx)
{
    ...
    diamnet::FuzzUtils::generateStoredLedgerKeys(firstGeneratedLedgerKey,
...
TransactionFuzzer::reduceNativeBalancesAfterSetup(AbstractLedgerTxn& ltxOuter)
{
    ...
    for (auto const& param : accountParameters)
    {
        ...
        auto ae = diamnet::loadAccount(ltx, account);
    ...
    AbstractLedgerTxn& ltxOuter)
{
    ...
    for (auto const& trustLine : trustLineParameters)
    {
        ...
        // issuer.
        auto tle = diamnet::loadTrustLine(ltx, account, asset);
...
{
    reinitializeAllGlobalStateWithSeed(1);
    diamnet::FuzzUtils::generateStoredLedgerKeys(mStoredLedgerKeys.begin(),
                                                 mStoredLedgerKeys.end());
