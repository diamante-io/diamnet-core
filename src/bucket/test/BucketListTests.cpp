...
{
    diamnet::uniform_int_distribution<uint32_t> dist;
...
TEST_CASE("BucketList snap reaches steady state", "[bucket][bucketlist][count]")
{
    ...
    for (uint32_t level = 0; level < BucketList::kNumLevels - 1; ++level)
    {
        ...
        diamnet::uniform_int_distribution<uint32_t> distLow(1, boundary - 1);
        diamnet::uniform_int_distribution<uint32_t> distHigh(boundary);
        for (uint32_t i = 0; i < 1000; ++i)
        {
