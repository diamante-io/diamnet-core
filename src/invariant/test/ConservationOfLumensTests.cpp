               ...
               int64_t netChange, bool updateTotalCoins)
{
    ...
    for (auto iter = entries.begin(); iter != entries.end(); ++iter)
    {
        ...
        if (iter + 1 != entries.end())
        {
            ...
            diamnet::uniform_int_distribution<int64_t> dist(maxDecrease,
                                                            maxIncrease);
...
updateBalances(std::vector<LedgerEntry> const& entries, Application& app)
{
    ...
    diamnet::uniform_int_distribution<int64_t> dist(
          ...
          "[invariant][conservationoflumens]")
{
    ...
    diamnet::uniform_int_distribution<int64_t> dist(0, INT64_MAX);
          ...
          "[invariant][conservationoflumens]")
{
    Config cfg = getTestConfig(0);
    cfg.INVARIANT_CHECKS = {"ConservationOfLumens"};
    diamnet::uniform_int_distribution<uint32_t> payoutsDist(1, 100);
    diamnet::uniform_int_distribution<int64_t> amountDist(1, 100000);
    ...
    for (uint32_t i = 0; i < 100; ++i)
    {
        ...
        diamnet::uniform_int_distribution<int64_t> deltaFeePoolDist(
            0, 2 * inflationAmount);
