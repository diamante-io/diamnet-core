...
FeeBumpTransactionFrame::commonValidPreSeqNum(AbstractLedgerTxn& ltx)
{
    ...
    if (!diamnet::loadAccount(ltx, getFeeSourceID()))
    {
                                     ...
                                     AbstractLedgerTxn& ltxOuter, bool applying)
{
    ...
    auto feeSource = diamnet::loadAccount(ltx, getFeeSourceID());
    if (!checkSignature(
...
{
    auto account = diamnet::loadAccount(ltx, getFeeSourceID());
    if (!account)
    {
...
DiamnetMessage
FeeBumpTransactionFrame::toDiamnetMessage() const
{
    DiamnetMessage msg(TRANSACTION);
