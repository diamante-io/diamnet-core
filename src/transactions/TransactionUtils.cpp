                            ...
                            LedgerTxnEntry& entry, int64_t amount)
{
    ...
    if (!diamnet::addBalance(newBalance, amount, tl.limit))
    {
        return false;
    }
                              ...
                              Asset const& asset)
{
    ...
    for (auto& offer : offers)
    {
        ...
        auto trustAcc = diamnet::loadAccount(ltxInner, account);
                                   ...
                                   SequenceNumber txSeqNum, uint32_t opIndex)
{
    ...
    for (auto const& poolTLKey : poolTLKeys)
    {
        ...
        {
            auto trustAcc = diamnet::loadAccount(ltxInner, accountID);
        ...
        auto redeemIntoClaimableBalance =
            ...
            if (loadSponsorship(ltxInner, cbSponsoringAccID))
            {
                auto cbSponsoringLtxAcc =
                    diamnet::loadAccount(ltxInner, cbSponsoringAccID);
...
struct MuxChecker
{
    ...
    void
    operator()(diamnet::MuxedAccount const& t)
    {
