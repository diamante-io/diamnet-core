...
getPRNGBytes(size_t n, diamnet_default_random_engine& engine)
{
...
static SecretKey
pseudoRandomForTestingFromPRNG(diamnet_default_random_engine& engine)
{
...
SecretKey::pseudoRandomForTestingFromSeed(unsigned int seed)
{
    ...
    // to guess PRNG-ness. It's intended for _deterministic_ use, when you want
    // "slightly random-ish" keys, for test-data generation.
    diamnet_default_random_engine tmpEngine(seed);
...
hash<diamnet::PublicKey>::operator()(diamnet::PublicKey const& k) const noexcept
{
    ...
    releaseAssert(k.type() == diamnet::PUBLIC_KEY_TYPE_ED25519);
