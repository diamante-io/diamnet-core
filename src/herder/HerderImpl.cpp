...
void
HerderImpl::setTrackingSCPState(uint64_t index, DiamnetValue const& value,
...
void
HerderImpl::newSlotExternalized(bool synchronous, DiamnetValue const& value)
{
    ZoneScoped;
...
void
HerderImpl::processExternalized(uint64 slotIndex, DiamnetValue const& value)
{
    ZoneScoped;
                             ...
                             std::optional<SecretKey> skToSignValue)
{
    ...
    DiamnetValue sv =
        makeDiamnetValue(txSet->getContentsHash(), closeTime, upgrades, sk);
                              ...
                              bool checkTrackingSCP)
{
    ...
    // the exact closeTime contained in the DiamnetValue with the best
    ...
    DiamnetValue newProposedValue = makeDiamnetValue(
...
DiamnetValue
HerderImpl::makeDiamnetValue(Hash const& txSetHash, uint64_t closeTime,
                             xdr::xvector<UpgradeType, 6> const& upgrades,
                             SecretKey const& s)
{
    ZoneScoped;
