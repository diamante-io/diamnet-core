...
cereal_override(cereal::JSONOutputArchive& ar, const xdr::opaque_array<N>& s,
                const char* field)
{
    xdr::archive(ar, diamnet::binToHex(diamnet::ByteSlice(s.data(), s.size())),
                 field);
}
...
void cereal_override(cereal::JSONOutputArchive& ar, const diamnet::PublicKey& s,
                     const char* field);

void cereal_override(cereal::JSONOutputArchive& ar,
                     const diamnet::MuxedAccount& muxedAccount,
                     const char* field);

void cerealPoolAsset(cereal::JSONOutputArchive& ar, const diamnet::Asset& asset,
                     const char* field);
...
template <typename T>
typename std::enable_if<std::is_same<diamnet::Asset, T>::value ||
                        std::is_same<diamnet::TrustLineAsset, T>::value ||
                        std::is_same<diamnet::ChangeTrustAsset, T>::value>::type
                ...
                const char* field)
{
    ...
    case diamnet::ASSET_TYPE_CREDIT_ALPHANUM12:
    {
        ...
        std::string code;
        if (asset.type() == diamnet::ASSET_TYPE_CREDIT_ALPHANUM4)
        {
            diamnet::assetCodeToStr(asset.alphaNum4().assetCode, code);
        }
        else
        {
            diamnet::assetCodeToStr(asset.alphaNum12().assetCode, code);
        }

        xdr::archive(ar, code, "assetCode");
        xdr::archive(ar, diamnet::getIssuer(asset), "issuer");
        ar.finishNode();
        break;
    }
    default:
