...
using uniform_u64 = diamnet::uniform_int_distribution<uint64_t>;
...
class SlidingWindowTester
{
  ...
  public:
    ...
    {
        mSlidingWindowSample.Seed(diamnet::gRandomEngine());
    }
    ...
    addSamplesAtFrequency(std::chrono::milliseconds timeStep, Args... args)
    {
        ...
        {
            uint64_t sample =
                static_cast<uint64_t>(dist(diamnet::gRandomEngine));
...
sampleFrom(Args... args)
{
    ...
    {
        sample.emplace_back(dist(diamnet::gRandomEngine));
    }
...
testCKMSSample(int const count, Args... args)
{
    ...
    {
        auto x = static_cast<int64_t>(dist(diamnet::gRandomEngine));
        values.push_back(x);
        hist.Update(x);
    }
