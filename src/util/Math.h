...
typedef std::minstd_rand diamnet_default_random_engine;

extern diamnet_default_random_engine gRandomEngine;

template <typename T>
T
rand_uniform(T lo, T hi)
{
    return diamnet::uniform_int_distribution<T>(lo, hi)(gRandomEngine);
}
...
#ifdef BUILD_TESTS
// This function should be called any time you need to reset diamnet-core's
