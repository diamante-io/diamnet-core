                      ...
                      public NonMovableOrCopyable
{
  public:
    ...
    void relayOrProcessResponse(DiamnetMessage const& msg, Peer::pointer peer);
    void relayOrProcessRequest(DiamnetMessage const& msg, Peer::pointer peer);
    void clearOldLedgers(uint32_t lastClosedledgerSeq);
    Json::Value const& getJsonResults();

    static std::string getMsgSummary(DiamnetMessage const& msg);
    DiamnetMessage makeSurveyRequest(NodeID const& nodeToSurvey) const;
  ...
  private:
    ...
    void broadcast(DiamnetMessage const& msg) const;
