...
Peer::recvMessage(DiamnetMessage const& diamnetMsg)
{
    ...
    if (!mApp.getLedgerManager().isSynced() && diamnetMsg.type() == TRANSACTION)
    {
    ...
    mApp.postOnMainThread(
        [weak, sm = DiamnetMessage(diamnetMsg), mtype = diamnetMsg.type(), cat,
...
Peer::recvRawMessage(DiamnetMessage const& diamnetMsg)
{
    ...
    releaseAssert(isAuthenticated() || diamnetMsg.type() == HELLO ||
                  diamnetMsg.type() == AUTH || diamnetMsg.type() == ERROR_MSG);
