...
If `diamnet-core fuzz` (or `afl-fuzz`) produces output such as 'Warning: AFL++
...
the tests). Then you can run `afl-fuzz -m 500 -M main -t 250 -i min-testcases -o fuzz-findings ./diamnet-core fuzz --ll ERROR --process-id 0 --mode=${FUZZER_MODE} @@` 
